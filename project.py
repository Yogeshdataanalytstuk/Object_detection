...
def calculate_iou(box1, box2):
    ...
    box2_area1 = (box2[2] - box2[0] + 1) * (box2[1] - box2[3] + 1)
    ...
    if box1_area1 + box2_area1 - intersection_area1 != 0:
...
def process(image):
    ...
    for r in results:
        ...
        for i in range(len(frame_left)):
            ...
            frame_left1=frame_left[i]
            frame_right1=frame_right[i]
            frame_left1=img_resize(org,image,frame_left1)
            frame_right1 = img_resize(org, image, frame_right1)

            #print("frame_left",frame_left)
            frame.append([[F,R],frame_left1,frame_right1])
    return frame
...
while True:
    ...
    for re, le, ri in values:
        x, y = re[0], re[1]
        cv2.rectangle(frame, x,y, (0, 255, 0), 2)

        left, right = le, ri
        cv2.rectangle(frame, (left[0], left[1]), (right[0], right[1]), (0, 0, 255), 2)
